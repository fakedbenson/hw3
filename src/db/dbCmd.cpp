...
DBAppendCmd::exec(const string& option)
{
   ...
   for(size_t i = 0; i < option.length(); ++i){
      if(option[i] == ' ') space = true;
      else if(option[i] != ' ' && space){
         op_num ++;
         space = false;
      }
   }
   if (!CmdExec::lexOptions(option, options, op_num))
         return CMD_EXEC_ERROR;
   if (!dbtbl) {
      cerr << "Error: Table is not yet created!!" << endl;
      return CMD_EXEC_ERROR;
   }
   if(op_num == 1) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);      

   data.resize(op_num-1);

   bool row = false;
   bool col = false;
   if (myStrNCmp("-Row", options[0], 2) == 0) row = true;
   else if (myStrNCmp("-Column", options[0], 2) == 0) col = true;
   else  return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[0]);

   for(size_t i = 1; i < options.size(); ++i){
      if (!myStr2Int(options[i], data[i-1])){
         if(options[i] == "-")
            data[i-1] = NONE;
         else{   
   ...
   if(row){
      if(op_num-1 < dbtbl.nCols()) {
         data.resize(dbtbl.nCols());
         for(size_t i = op_num-1; i < dbtbl.nCols(); ++i) data[i] = NONE;
      }
      if(op_num-1 > dbtbl.nCols()) data.resize(dbtbl.nCols());
      dbtbl.addRow(data);
   }
   else if(col){
      if(op_num-1 < dbtbl.nRows()) {
         data.resize(dbtbl.nRows());
         for(size_t i = op_num-1; i < dbtbl.nRows(); ++i) data[i] = NONE;
      }
      if(op_num-1 > dbtbl.nRows()) data.resize(dbtbl.nRows());
      dbtbl.addCol(data);
   }
...
{  
   ...
   string opt = "";
   if(op_num < 1) return CmdExec::errorOption(CMD_OPT_MISSING, opt);
   if (!CmdExec::lexOptions(option, options, op_num))
         return CMD_EXEC_ERROR;
   ...
   bool summary = false;
   int r, c;
   if(options.size() > 1 && myStr2Int(options[0], r))  {if (myStr2Int(options[0], r) && myStr2Int(options[1], c))  data = true;}
   else if (myStrNCmp("-Row", options[0], 2) == 0) row = true;
   else if (myStrNCmp("-Column", options[0], 2) == 0) col = true;
   else if (myStrNCmp("-Table", options[0], 2) == 0) table = true;
   else if (myStrNCmp("-Summary", options[0], 2) == 0) summary = true;
   else  return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[0]);

   if(data){
      if(op_num > 2) return CmdExec::errorOption(CMD_OPT_EXTRA, options[2]);
      if (!checkRowIdx(options[0], r) || !checkColIdx(options[1], c))   return CMD_EXEC_ERROR;
      if (dbtbl[r][c] != NONE )  cout << dbtbl[r][c] << endl;
      else cout << "null" << endl;
   }
   else if(row){
      ...
      else if (!checkRowIdx(options[1], r)) return CMD_EXEC_ERROR;
      for(size_t i = 0; i < dbtbl[r].size(); ++i){
         if(dbtbl[r][i] != NONE) cout << dbtbl[r][i];
         else cout << ".";
         if(i != dbtbl.nCols()-1)  cout << " ";
      }
      cout << endl;
   }
   else if(col){
      ...
      else if (!checkRowIdx(options[1], c)) return CMD_EXEC_ERROR;
      dbtbl.printCol(c);
      cout << endl;
   }
   else if(table){
